...
const getRealJid = (ctx) => {
  try {
    ...
    if (rjid && rjid.endsWith('@lid')) {
      ...
      console.log('ℹ️ LID sin phone, respondo al @lid:', rjid)
      return rjid
    }
...
const obtenerOpcionesEnumeradas = (campo, lista) => {
  const unicas = [...new Set(lista.map(i => i[campo]))]
...
const filtrarPorCriterios = (supervisor, vendedor, dia) =>
  data.filter(i => i.supervisor===supervisor && i.vendedor===vendedor && i.dia===dia)
      .map(i => `${i.codigo} - ${i.razonSocial}`).join('\n')
...
/* ====== REFERENCIA AL PROVIDER PARA ENVIAR DIRECTO ====== */
let adapterProvider
...
const flujoConsulta = addKeyword(EVENTS.ACTION)
  ...
  .addAnswer('', { capture: true }, async (ctx, tools) => {
    ...
    if (resultados.length > 5) { await sendTo(jid, `Tené en cuenta que:\n\n${resultados[5]}`) }
  })
...
const constConsulta = addKeyword(EVENTS.ACTION)
  .addAction(async (ctx) => {
    ...
    const jid = getRealJid(ctx); if (!jid) return
    await sendTo(jid, 'Aca la idea es agregar consultas extra')
  })
...
const main = async () => {
  ...
  adapterFlow = createFlow([ menuFlow, constMenu, constAACC, constConsulta, constPregunta,
